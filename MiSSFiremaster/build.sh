...
function delete_commons {
	...
	if [[ -f $BANK_SERVICES/$DOCKER_YML_ORIGINAL ]]; then
		...
		cp $DOCKER_YML $BANK_SERVICES/$DOCKER_YML
	else
...
function build_docker_images {
	...
	cd $MISSFIRE_CLIENT_DOCKER_IMAGE
	docker build -t bankmodeldefault .


	echo "Building Docker image: 'missfire'."
	cd ../$MISSFIRE_SERVICES
	docker build -t missfire .
